...
def update_corns(new_corns):
    if len(new_corns) > 0:
        new_corn_rects = []
        for nc in new_corns:
            x_pos = nc[2] * rect_size
            y_pos = nc[1] * rect_size
            new_corn_rect = pygame.draw.rect(win, color_corn, (x_pos, y_pos, rect_size, rect_size))
            new_corn_rects.append(new_corn_rect)
        pygame.display.update(new_corn_rects)
